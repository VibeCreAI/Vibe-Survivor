 ...
 * Confirmation dialog for restarting the game
...
/**
 * RestartConfirmationModal - Asks for confirmation before restarting
 */
export class RestartConfirmationModal extends Modal {
    constructor(id = 'restart-confirmation-modal') {
    ...
    /**
     * Initializes restart confirmation modal elements
     */
    init() {
        const result = super.init();
        if (result) {
            this.yesButton = this.element.querySelector('#restart-confirm-yes');
            this.noButton = this.element.querySelector('#restart-confirm-no');
    ...
    updateButtonHighlight() {
        ...
        if (this.keyboardUsed && this.navigableButtons[this.selectedIndex]) {
            ...
            selected.style.boxShadow = '0 0 15px rgba(255, 136, 68, 0.8)';
            selected.style.borderColor = '#ff8844';
        }
    }
     ...
     */
    addNavigationStyles() {
        if (document.getElementById('restart-confirm-nav-styles')) return;

        const style = document.createElement('style');
        style.id = 'restart-confirm-nav-styles';
        style.textContent = `
            .restart-confirmation-buttons .menu-selected {
                box-shadow: 0 0 15px rgba(255, 136, 68, 0.8) !important;
    ...
    /**
     * Shows restart confirmation modal
     */
    onShow() {
    ...
    /**
     * Hides restart confirmation modal
     */
    onHide() {
    ...
    /**
     * Updates localized text for restart confirmation modal
     */
    updateLocalization() {
        ...
        if (title) title.textContent = t('restartConfirm');

        const warning = this.element.querySelector('p');
        if (warning) warning.innerHTML = t('restartWarning');

        if (this.yesButton) this.yesButton.textContent = t('yesRestart');
        if (this.noButton) this.noButton.textContent = t('noContinue');
    }
}
