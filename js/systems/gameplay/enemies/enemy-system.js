...
export class EnemySystem {
    ...
    spawnEnemy(params) {
        ...
        if (bossesKilled === 0) {
            ...
            bossScaling = 1 + bossesKilled * 1.0; // 100% health scaling per boss defeated
        }

        const totalHealthMultiplier = config.health * timeScaling * bossScaling;
        const totalDamageMultiplier = config.contactDamage * (1 + (bossesKilled || 0) * 0.3); // 30% damage per boss
