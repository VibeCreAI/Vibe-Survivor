# Complete Refactoring Execution Prompt (No Agents Version)

**CRITICAL INSTRUCTIONS FOR AI:**

Execute the complete refactoring plan outlined in `refector.md` autonomously through all 10 phases. Work continuously without stopping until all phases are complete and ready for human testing. Do NOT use any subagents - handle all tasks directly.

---

## EXECUTION STRATEGY

Work through all phases sequentially, handling each task directly without delegating to specialized agents. Maintain full context throughout the entire refactoring process.
...
4. **Review**: Review entire module structure for consistency and completeness

5. **Validate**: Ensure no functionality was lost in migration
...
3. **Review**: Review final code quality for consistency and best practices
...
1. **Context Management**:
   - Maintain full awareness of what's been extracted and what remains
   - Keep track of dependencies as they're discovered
...
4. **Error Handling**:
   ...
   - Document any issues that need human intervention
...
### Checklist Updates
**CRITICAL**: As you complete each task, update the checkboxes in `refector.md` by changing `- [ ]` to `- [x]`. This allows the human to track real-time progress.

Use the Edit tool to update `refector.md` after completing each phase or major task group.

### Status Updates
As you work through each phase, provide brief status updates:

```
✅ Phase 0: Analysis Complete
   - Dependency map created (234 dependencies mapped)
   - Extraction order determined
   - 91 sprite references cataloged

⏳ Phase 1: Scaffolding (In Progress)
   - Created 47 placeholder files
   - Directory structure established
   ...

❌ Phase X: Blocked
   - Issue: Circular dependency between X and Y
   - Resolution: Refactoring to break dependency
```
...
3. **Performance Metrics**:
   ...
   - Any performance impacts detected
   - File size comparisons (before/after)

4. **Ready for Human Testing**:
...
- **HANDLE ERRORS GRACEFULLY** - if blocked, explain clearly and propose solution
...
3. **Mark checkboxes in `refector.md` as you complete tasks** using the Edit tool
4. Work through all 10 phases without stopping
...
## COMPARISON NOTES FOR TESTING

This version is designed to test:
- Single AI instance capability vs multi-agent approach
- Context window management without agent delegation
- Continuous execution without specialized expertise
- Performance and efficiency metrics
- Error handling and recovery without specialized debuggers
- Documentation quality without specialized writers

Compare results with agent-based version for:
- Completion time
- Code quality
- Error rate
